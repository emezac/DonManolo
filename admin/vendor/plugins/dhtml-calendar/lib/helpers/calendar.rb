  ...
  module DhtmlCalendarHelper
    ...
    def active_scaffold_input_calendar(column, options)
      ...
      if defined? ActiveScaffold::Helpers
        ...
        options[:help_string] ||= "mm/dd/yyyy"
        calendar_options[:help_string] = as_(options[:help_string])
...
END
        ...
        text_field_with_example = ""
        if defined? ActiveScaffold::Helpers
          text_field_with_example = "new TextFieldWithExample('#{input_field_id}','#{calendar_options[:help_string]}');" if calendar_options[:help_string] and calendar_options[:help_string].length > 0
        end
